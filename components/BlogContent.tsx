...
export default function BlogContent({ post, preview }: PostProps) {
    ...
    {!preview ? <span className={"flex flex-col max-w-lg w-full justify-between align-middle items-center"}>
        <Reactions post={post as any}/>
        <span className={"text-3xl max-sm:text-2xl font-montserrat text-primary"}>Comments</span>
    </span> : ""}
    </main>)
}