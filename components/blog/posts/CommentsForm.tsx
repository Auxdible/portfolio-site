...
export default function CommentsForm({ postId }: CommentsProps) {
    ...
    return (<div className={"flex flex-col justify-start w-full gap-3"}>  
        <span className={"text-3xl max-sm:text-3xl font-montserrat text-title max-md:flex max-md:flex-col"}>Create Comment {session?.user ? <span className={"text-xl max-sm:text-lg font-roboto dark:text-gray-400 text-gray-600"}>(commenting as {session.user.name})</span> : "" }</span>
