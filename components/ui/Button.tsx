...
export function Button({ children, href, className, ...props }: ButtonProps & AnchorHTMLAttributes<HTMLAnchorElement>) {
    ...
    return <a href={href} {...hoverable(setHovered)} {...props} className={"border-2 cursor-pointer border-solid relative border-gray-800 dark:border-gray-200 hover:border-gray-300 dark:hover:border-gray-900 transition-colors px-4 py-2 rounded-full group font-raleway " + className}>
    ...
    <span className="z-20 relative transition-all duration-700 group-hover:text-theme text-reverse select-none group-hover:-translate-y-1">
        {children}
    </span>
    </a>
}