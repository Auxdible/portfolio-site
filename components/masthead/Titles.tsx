...
export default function Titles({ titles, duration }: { titles: string[], duration: number }) {
    ...
    return (<span className={"overflow-hidden relative w-full block"}>
        ...
        {titles.map((i, index) => <p key={index} className={`text-4xl ease-out duration-500 max-md:text-4xl max-sm:text-2xl max-[412px]:text-xl font-montserrat py-2 transition-all flex-grow-0 flex-shrink-0 w-[30rem] max-lg:w-screen`} style={{ transform: `translateX(-${state*100}%)`}}>{i}</p>)}
        </div>
    </span>)
}