...
export default function Tool({ Icon, title, description, certifications, delay }: ToolProps) {
    ...
    return (
        <div onClick={() => setToggled(!toggle)} className={`cursor-pointer relative mx-auto flex flex-col gap-2 text text-lg items-center text-center font-extralight group dark:bg-black bg-white w-full ${toggle ? "h-80 max-sm:h-fit" : `h-32 hover:h-80${isIAB || xs ? " hover:h-fit " : " "}`} transition-all py-2 rounded-xl`}>
        ...
        >
        <div className={`absolute -z-10 bg-gradient-radial dark:from-black from-white dark:via-black via-white to-transparent w-16 h-16 -translate-x-1/2 left-1/2 md:h-12 ${toggle ? "-translate-y-8 scale-110" : "group-hover:-translate-y-8 group-hover:scale-110"} delay-75 transition-all`}></div><Icon className={`text-5xl text-black dark:text-white ${toggle ? "-translate-y-8 scale-110" : "group-hover:-translate-y-8 group-hover:scale-110"} delay-75 transition-all`}/></motion.span>
        <h1 className={"text-3xl max-md:text-2xl font-montserrat text-title"}>{title}</h1>
        <section className={`flex flex-col gap-2 justify-between items-center flex-grow ${toggle ? "scale-100 flex-shrink" : "group-hover:scale-100 scale-0 group-hover:flex-shrink flex-shrink-0"} transition-all overflow-hidden`}>
