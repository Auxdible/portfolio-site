...
export default function Project({ project }: ProjectProps) {
    ...
    whileInView={{ opacity: 1, transform: "translateY(0)" }} transition={{ duration: 1.5 }} viewport={{ once: true }} className={"flex flex-col mx-auto justify-center items-center text-center w-full max-w-lg mb-40"}>
          ...
          onMouseMove={({clientX: x, clientY: y}) => (set({xys: calc(x, y, 1.0)}))}
    ...
    whileInView={{ opacity: 1, transform: "translateX(0)" }} transition={{ duration: 1.5 }} viewport={{ once: true }} href={project.project_source_url} className={"flex group relative items-center justify-center flex-1 flex-grow flex-shrink mx-auto p-2 hover:px-3 transition-all font-roboto text-lg"}>
      ...
      <span className={"z-10 group-hover:text-title"}>View Source</span></motion.a> : ""}
          {project.project_website_url ? <motion.a initial={{ opacity: 0, transform: "translateX(8rem)" }}
    whileInView={{ opacity: 1, transform: "translateX(0)" }} transition={{ duration: 1.5 }} viewport={{ once: true }} href={project.project_website_url} className={"flex group relative z-0 items-center justify-center flex-1 flex-grow flex-shrink mx-auto  p-2 hover:px-3 transition-all rounded-lg font-roboto text-lg"}>
       <div className={"absolute inset-0 dark:bg-black bg-white rounded-lg"}></div>
       <div className={"absolute -z-10 -inset-[1px] bg-gradient-to-t from-orange-400 to-red-500 rounded-lg group-hover:-inset-[2px] transition-all"}></div>
       <span className={"z-10 group-hover:text-title"}>View Website</span></motion.a> : ""}
          </span>
