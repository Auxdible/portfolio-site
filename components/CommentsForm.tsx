...
import { BsChatText, BsHandThumbsUp, BsMessenger } from "react-icons/bs";
...
export default function CommentsForm({ postId }: CommentsProps) {
    ...
    return (<div className={"flex flex-col justify-start w-full gap-3"}>  
        <span className={"text-3xl max-sm:text-2xl font-montserrat text-primary max-md:flex max-md:flex-col"}>Create Comment {session?.user ? <span className={"text-xl max-sm:text-lg font-roboto dark:text-gray-400 text-gray-600"}>(commenting as {session.user.name})</span> : "" }</span>
        ...
        <button onClick={() => handleSubmit()} className={"self-center flex font-roboto justify-center items-center gap-2 text-center text-black dark:text-white p-2 border-2 dark:border-orange-400 border-orange-600 border-opacity-40 rounded-2xl w-fit"}><BsChatText/> Comment</button>
    </div>)
}